...
def parse_classification_percents(report_file):
    ...
    with open(report_file) as f:
        for line in f:
            ...
            # The 'root' percentage is the total classified amount
            if name == "unclassified":
                percent_unclassified = pct
            elif name == "root":
                percent_classified = pct  # Use root for total classified
    
    # Fallback/safety check, assuming percentages are based on total reads
...
def prepare_input(input_source, output_folder):
    ...
    if os.path.isfile(input_source):
        ...
        # Dump to FASTQ, splitting paired-end reads if necessary
        # The temporary directory is used for the dump process to avoid clutter
...
def run_kraken_bracken(fastq_files, kraken_db, outdir, prefix, read_len):
    ...
    # Determine if paired-end is needed
    ...
    # Convert TSV (Tab-Separated) to CSV using pandas
    try:
...
def merge_metag_metat(bracken_g_csv, bracken_t_csv,
    ...
    # Define columns to keep and ensure presence of required columns (for merging only)
...
def main():
    print("\n Welcome to the W&M IGEM 2025 Full Analysis Pipeline (Python Edition)! \n")
    print("This script performs SRA download (if needed), runs Kraken2/Bracken, and merges the results.\n")
    ...
    metaT_input = input("Enter metatranscriptomic SRA accession or FASTQ file path (-r): ").strip()
    if not metaT_input:
        print("Metatranscriptomic input is required.")
        sys.exit(1)

    krakendb = input("Enter path to Kraken2 database (-d): ").strip()
    if not krakendb:
        print("Kraken2 database path is required.")
        sys.exit(1)

    outdir = input("Enter output directory [default: results]: ").strip() or "results"
    
    read_len = input("Enter read length for Bracken (-r) [default: 150]: ").strip() or "150"
    if not read_len.isdigit():
        print("Bracken read length must be an integer.")
        sys.exit(1)

    # 2. Setup Output Directory
    # We use a placeholder for the folder name until the actual accession/filename is resolved
    
    # Determine accession IDs for output naming and setup folders
    # Use simple split for initial folder name, will refine later
    ...
    # 3. Prepare Inputs (Download SRA or use local files)
    try:
    ...
    compute_similarity(merged_file, overlap_file, folder_path, final_name)
